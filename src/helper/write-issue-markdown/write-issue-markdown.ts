...
export function createIssueMarkdown(issueContents: IssueContents[]): Result<string, Error> {
    ...
    const bodyContents = issueContents.filter(
        (content) =>
            ...
            content.title !== LABEL_KEY &&
            content.title !== ASSIGN_KEY &&
            content.title !== PARENT_KEY,
    );
    ...
    if (parentContent && parentContent.contents.trim().length > 0) {
        markdownLines.push(`parent: ${parentContent.contents}`);
    }
