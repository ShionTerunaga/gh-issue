...
export function parseDraftIssue(filePath: string, cwd = process.cwd()): DraftIssue {
    ...
    const parentIssueNumber = parentMatch ? Number.parseInt(parentMatch[1].trim(), 10) : undefined;

    if (!titleMatch) {
        throw new Error(`Missing title in front matter: ${filePath}`);
    }

    if (parentMatch && (parentIssueNumber === undefined || Number.isNaN(parentIssueNumber))) {
        throw new Error(`Invalid parent issue number in front matter: ${filePath}`);
    }
