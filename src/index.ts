...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "init") {
    try {
      const templatePaths = await initIssueTemplates(args);
      for (const templatePath of templatePaths) {
        console.log(`Created ${templatePath}`);
      }
    } catch (error) {
      if (error instanceof Error && "code" in error && error.code === "EEXIST") {
        console.error("Issue templates already exist. Re-run with --force to overwrite them.");
        process.exitCode = 1;
        return;
      }
  ...
  if (command === "template") {
    try {
      const templatePath = await createIssueTemplate(args);
      console.log(`Created ${templatePath}`);
    } catch (error) {
      if (error instanceof Error && "code" in error && error.code === "EEXIST") {
        console.error("Issue template already exists. Re-run with --force to overwrite it.");
        process.exitCode = 1;
        return;
      }
