...
import { createIssueTemplate, createIssueTemplateYaml, initIssueTemplates } from "./index";
...
describe("createIssueTemplateYaml", () => {
    ...
    it("reads Japanese templates", async () => {
        ...
        await expect(createIssueTemplateYaml("feature_request", "ja")).resolves.toContain(
            "機能要望",
        );
    });
    ...
    it("supports short template aliases", async () => {
        ...
        await expect(createIssueTemplateYaml("feature")).resolves.toContain(
...
describe("initIssueTemplates", () => {
    it("writes bug and feature templates under .github/ISSUE_TEMPLATE", async () => {
        ...
        await expect(readFile(templatePaths[1], "utf8")).resolves.toContain(
